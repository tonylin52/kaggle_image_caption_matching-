...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, split='train', is_train=True):
        ...
        self.image_id2idx = json.load(open(args.imgid2idx_file))  # img_id as string
        if args.add_od_labels:
            label_data_dir = op.dirname(self.img_file)
            
            self.label_tsv = TSVFile(args.label_file)
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, 
        ...
        segment_ids = torch.tensor(segment_ids, dtype=torch.long)#token_type_ids
    ...
    def get_image(self, image_id):
        ...
        num_boxes = int(row[1])
        feature=row[2]
        features = np.frombuffer(base64.b64decode(feature),
...
def main():
    ...
    parser.add_argument("--data_dir", default='./output/X152C5_test/inference/vinvl_vg_x152c4', type=str, required=False,
                        help="The input data dir with all required files.")
    parser.add_argument("--img_feat_file", default="features.tsv", type=str, required=False,
                        help="The absolute address of the image feature file.")
    parser.add_argument("--imgid2idx_file", default="imageid2idx.json", type=str, required=False,
                        help="")
    parser.add_argument("--label_file", default="predictions.tsv", type=str, required=False,
                        help="")
    ...
    parser.add_argument("--num_captions_per_img_train", default=1, type=int,
                        help="number of positive matched captions for each training image.")
    parser.add_argument("--num_captions_per_img_val", default=1, type=int,
                        help="number of captions for each testing image.")
    parser.add_argument("--caption_file", default="test_captions.pt", type=str,
                        help="captions file")
